...
**Answers**
<br>
Q1: to explain at least 50% of the variance in this data,the first 6 components are needed.  
<br>
Q2: for the first principal component of the portfolio, the highest weights are: 

```{r echo=TRUE}
i=0
for (n in 1:nrow(Rotated_Factors)) if (!is.na(Rotated_Factors[n,1])) i=i+1


sorted_rows <- sort(Rotated_Factors[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors <- Rotated_Factors[sorted_rows,]

as.data.frame(show_data[1:i,2], row.names =rownames(show_data[1:i,]))

```
  
<br>
Q3 The first 10 components are portfolios of futures which behave in a similar way. the factors could be equivalent to the betas of the futures in the portfolios. We would not want to use these components as actual portfolios because they are not diversified. ideally we would try to build a portfolio with futures in each of the components. 

<br>
Q4 
Definition of the subset data:
```{r}
...
*
1. L262 is initializing the vector that will ultimately give the coefficients that we will use to build our portfolio
L264 is generating a sequence of numbers from 10 to 250 with jumps of 20
L265,266 is a loop doing for every number of the above defined sequence, modifies the final signal to change the sign of each coefficient according to the sign of the moving average function on that given sequence coefficient
L268 is applying a function and transposing the matrix from columns to lines. The functions it is applying L268-273,
it is normalizing the (scaling down to 1) the sum of all the futures provided that the sum is not 0
L274 is naming the columns of our coefficient vector
L276  shifts the coefficients to take the data from 2 days ago and cleans the vector to make sure that there are no empty coefficients that woudl create an error
L278 is summing the strategy from each future into a total strategy
L279 labels the futures in the total strategy
*
*
2. 
*
