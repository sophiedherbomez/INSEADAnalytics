...
**Your Answers here:**
<br>1. in dataSet1.R, mytickers l8 is defined as a vector of two components, SPY and AAPL. The <br>loop "for " l20 gets the infos for one component after the other
<br>2. > AAPL = StockReturns[,"AAPL"]
<br> round(100*sum(AAPL),1)
<br>[1] 596.5
<br> round(100*mean(AAPL),3)
<br>[1] 0.148
<br> round(100*sd(AAPL),3)
<br>[1] 2.389
<br>3. for any company, find its ticker symbol for the <br>environment you're in (here yahoo!), for yahoo! it's YHOO, <br>and add it to the vector myticker and run the code again
<br> cumulative return yahoo 284.4
<br>average return yahoo 0.07
<br>standard deviation 3.019
```{r eval = FALSE, echo=TRUE, fig=TRUE}
source("dataSet1.R")
 YHOO = StockReturns[,"YHOO"]
 round(100*sum(YHOO),1)
 round(100*mean(YHOO),3)
 round(100*sd(YHOO),3)
pnl_plot(YHOO)
```
<hr>
...
**Your Answers here:**
<br>1. in dataset1.R change the start date to 2000-12-29 l9. We can only get it from 2001-01-02 because there is no trading on 2001-01-01 and the last day the stock exchange was open was 2000-12-29
<br>
...
pnl_plot(SPY)
```
<br>2. adapt StartDate and create EndDate to get the wished <br>data range (adapt using from StartDate to EndDate), <br>adapt myticker to have the symbols of the various <br>companies we want, and create the equally weighted <br>portfolio portfolio = apply(StockReturns,1,mean). Then <br>plot it.
<br>
...
**Your Answers here:**
<br>1. it gives the number of line in the cvs file
<br>2. myData and stockReturns have different formats, they cannot be added together
<br>
<br>
